...
import { drawHulls, drawRings, drawBadges, drawShapePath, applyColorOverride } from '../../../graphPrimitives';
import { buildForceSimulation } from '../../../layouts/forceLayout';
import { computeStaticPositions as radialPositions } from '../../../layouts/radialLayout';
import { computeStaticPositions as hierarchicalPositions } from '../../../layouts/hierarchicalLayout';
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      for (const node of nRef.current) {
        ...
        // Workspace-declared node.color is handled natively by resolveNodeColor
        // through the resolveColor callback. Query highlights are now drawn
        // via the annotation snapshot's rings (z-order 4 batch below).
