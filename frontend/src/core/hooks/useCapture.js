...
export function useCapture() {
  ...
  const visibleEdges = useMemo(
    () => (data.graph.edges || []).filter(e => {
      ...
      if (sel.hiddenEdges.has(e.id)) return false;
      if (filters.hiddenEdgeTypes.size > 0 && e.type && filters.hiddenEdgeTypes.has(e.type)) return false;
      return true;
    }),
    [data.graph.edges, sel.hiddenNodes, sel.hiddenEdges, filters.hiddenEdgeTypes]
  );
  ...
  return {
    ...
    forceParams: filters.forceParams, setForceParams: filters.setForceParams,
    frozen: filters.frozen, setFrozen: filters.setFrozen,
    reheatTick: filters.reheatTick, incReheat: filters.incReheat,
