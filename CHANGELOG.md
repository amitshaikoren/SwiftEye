...
- **Protocol boundary checker contract** — protocol field modules can now define an optional `check_boundary(flow_state, ex) → bool` function, auto-discovered alongside `init/accumulate/serialize`. Allows application-layer protocols to contribute session split signals without modifying `sessions.py`.
