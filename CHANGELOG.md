...
### v0.20.4 — April 2026
- **Fix: animation isolate now filters frames, not just edges** — previously, toggling "Isolate" in the animation pane only hid edges that didn't connect two spotlight nodes, but the timeline still played through every event in the capture (including sessions that involved non-spotlight neighbours). The slider, history panel, frame counter, and play loop all reflected the full event list, so the visualisation showed isolation while the playback machinery still iterated over discarded frames. Reported by user: "isolate feature in animation mode still has all the frames that belong to other connections".
- **`isIsolated` lifted into `useAnimationMode`** — the toggle state now lives in the hook instead of `AnimationPane` local state. The hook stores the unfiltered API response as `rawEvents`, derives `animEvents` (memoised) by filtering to spotlight↔spotlight events when isolated, and exposes `animEvents` as the single source of truth. `frameState`, `currentEvent`, `totalFrames`, `animTimeRange`, `eventsRef`, the play timer, and all transport callbacks (`goToFrame`, `stepForward`, `stepBackward`, `goToEnd`, `togglePlay`) operate on the effective list. A clamp effect shrinks `animFrame` when the effective list shrinks (e.g. toggling isolate on mid-playback). `isIsolated` is reset on `startAnimation` and `stopAnimation`. `AnimationPane` consumes `isIsolated`/`setIsIsolated` from props (wired through `c.isIsolated` in `App.jsx`); the now-redundant `if (isIsolated && (!spotSet.has(e.src) || !spotSet.has(e.dst)))` line in `visibleEdges` was removed since events are pre-filtered upstream.
- **`/add_to_roadmap` slash command rewritten cache-first** — `~/.claude/commands/add_to_roadmap.md`. The old version read `ROADMAP.md` lines 1–87 and wrote both the row and the detail block directly into `ROADMAP.md`, which violates the docs-second flush model (mid-session edits to disk-tier docs). The new version reads `ROADMAP.ai.md` for the status table, adds the row to that mirror, and stashes the full detail block in `SESSION.md → Pending flush` so the end-of-session flush is what actually inserts it into `ROADMAP.md`. Explicitly forbids touching `ROADMAP.md` mid-session and limits edits to `ROADMAP.ai.md` + `SESSION.md`.
- **`docs/METHODOLOGY.md` gitignored** — added to `.gitignore`. The methodology doc is private dogfood material, not redistributable, and the file was never committed (the repo just hadn't excluded it yet).
- **METHODOLOGY.md intro rewritten** — the old intro framed the methodology as being about "token efficiency". That was wrong about the goal. The new intro frames it as **scalability and cross-session knowledge retention**: keeping an LLM coherent with the project's principles, philosophy, and architectural constraints as the codebase grows past a few thousand lines. As projects scale, an LLM working on them starts losing the plot — forgetting invariants, re-violating warned-about rules, picking the wrong layer for new logic, drifting from the philosophy. The methodology is a discipline for retaining that knowledge. Token efficiency is one ingredient (you can't retain knowledge you can't afford to read), but it's the means, not the end.

### v0.20.3 — April 2026
- **Animation isolate toggle** — new "Isolate" button in the animation pane header. When on, only edges between two spotlight nodes are shown (visual filter only at the edge level). v0.20.4 makes this filter the timeline events as well.
- **Live protocol filter** — animation pane now responds to the global protocol filter from `useFilterContext()`. Edges whose protocol isn't in the enabled set are hidden. When zero protocols are selected, no edges render (matching graph view behaviour).
- **None=no edges** — clarified the empty-protocol-set semantics: zero selected protocols means zero edges, not "all edges".
