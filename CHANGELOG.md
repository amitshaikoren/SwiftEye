...
- **Phase 2 deferred** — backend persistence (workspace save), pluggable suggested-edge plugins (`event-suggested-edges-pluggable` roadmap item added), Phenomena (typed reusable event templates — needs real-usage data first), edge-label edit wired to TimelineGraph edge detail card. The full Opus design plan lives at `docs/plans/event-type-system.md`.
