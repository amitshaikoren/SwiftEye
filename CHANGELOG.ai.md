...
v0.24.0 | 2026-04-10 | fix | multi | QOL bug-fix batch: (1) alerts-live-load-bug — re-fetches alerts after first graph response (build_analysis_graph_and_run fires lazily on first /api/graph, so loadAll() was always getting empty store.alerts; fix: re-fetch in fullGraphRef first-set branch). (2) schema-dialog-in-app — SchemaDialog now also rendered in loaded App view, not just upload screen. (3) manual-type-override — backend returns detection_failed+available_adapters instead of 400 when no adapter matches; new force_adapter Form field accepted by /api/upload; frontend TypePickerDialog lets user pick the adapter and retries. (4) detail-panel-polish — removed "Flag" text from NodeDetail + EdgeDetail flag buttons (icon only, tightened padding). (5) animation-direction-mismatch — build_node_session_events now uses initiator_ip/responder_ip (TCP SYN-determined) when available, falling back to src_ip/dst_ip. (6) timeline-graph drag-render bug — onNodePointerMove now calls setTick() so dragged nodes move without needing a zoom/pan event.
