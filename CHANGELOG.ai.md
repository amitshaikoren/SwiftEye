...
v0.27.7 | 2026-04-14 | docs | all | docs audit: DEVELOPERS.md §9 API table (investigation, query, edge-sessions, paths, animation, upload/confirm-schema, edge-fields, GET metadata); analysis/→data/ path fixes throughout; HANDOFF.md §2 directory tree (routes/, llm/, storage/, services/, data/, expanded components+hooks); HANDOFF.md API table complete; README Features (alerts, LLM panel, animation, query translation)
v0.27.6 | 2026-04-13 | feat | frontend | graph direction viz (arrowhead toggle in Graph Options, default off, midpoint arrowhead src→dst); graph legend overlay (GraphLegend.jsx bottom-left canvas overlay syncs with active nodeColorMode + edgeColorMode, NODE_LEGENDS/EDGE_LEGENDS extracted to graphLegendData.js); export HTML enriched hover (nodes: +sessions +topProtos; edges: +sessions +protocols, edge hit detection via point-to-segment distance, edge hover tooltip)
v0.27.5 | 2026-04-13 | fix | frontend | 6 bugs: investigation timeline nodes visible immediately after drop; timeline node/edge editing (label, color, annotation, severity); edge hover tooltips; session flag icon-only; edge flag modal source/target resolve from d3 objects; session title format {ip}--proto→{ip}; Timeline panel removed from nav, session_gantt unblocked in Research
v0.27.4 | 2026-04-13 | docs | project | add CONTRACTS.md — full extension contract reference for all 12 extension points (adapters, protocol fields, edge fields, plugins, detectors, analyses, research charts, LLM providers, query contract, wire types, session sections, source capabilities)
v0.27.3 | 2026-04-13 | fix | frontend | rebuild fixes: sticky header (flex chain: wrapper flex-col + SessionDetail flex:1 minHeight:0, no height:100%); LLM settings moved from global SettingsPanel into LLMInterpretationPanel inline (⚙ toggle, useSettings called internally, onOpenSettings prop removed); AnalysisPage no longer imports useSettings; v0.27.2 changes now actually in dist (prior build was pre-commit).
v0.27.2 | 2026-04-12 | fix+feat | frontend+backend | quick-wins batch: logo click now calls switchPanel('stats') not clearAll (navigates home from all panels); plotly/sqlglot import failures degrade gracefully (try/except, _PLOTLY_AVAILABLE flag, runtime error not startup crash); SessionDetail sticky header (flex col, header pinned, content scrolls); EdgeDetail port pairs display (src → dst in one row); d3 force sim tuning (charge -280→-180, distanceMax 500→400, link distance 160→130, alphaDecay 0.025); animation back-button breadcrumb in right panel when animActive + detail open; TimelineStrip animation frame cursor (orange tick marker); animation node positions persist across full-width panel switches via savedPositionsRef in App.jsx.
