...
- **PySpark translator** — `parse_pyspark()` translates PySpark DataFrame filter expressions (`.filter()`, `.where()`, `col()` comparisons, `.contains()`, `.startswith()`, boolean combinators) into the JSON query contract via Python `ast` module. 27 tests. Frontend dialect selector updated from "Spark SQL" to "PySpark".
- **Field reference panel** — query builder shows available fields grouped by type (Numeric, Sets, Flags, Text) in dialect-appropriate syntax.
- **Node statistics** — collapsible section in NodeDetail with source ports, dest ports, top neighbors (clickable), protocol breakdown as horizontal bar charts.
- **Query highlight clearing** — click empty canvas to clear, visible clear button.
- **Edge click fix** — D3 force mutation and protocol suffix mismatch resolved for query result edge clicks.
- **Time sort** — sessions sortable by start time in both SessionsTable and AnalysisPage.
- **HTTP cookie extraction** — tshark HTTP adapter extracts Cookie and Set-Cookie headers.
...
- [ ] **Node color mode selector** — currently node color is hardcoded to private (green) vs external (pink) based on RFC1918. This should be a dropdown in Graph Options letting the user choose what drives node color:
  - **Address type** (current default) — private/external/multicast/loopback. Plus user-defined CIDR → color/label rules for non-standard ranges (e.g. `100.70.0.0/16 → blue "Lab"`). Import from researcher metadata JSON or presets (RFC1918, cloud ranges).
  - **OS** — color by detected OS family (Windows=blue, Linux=green, macOS=gray, etc.). Data already in `os_guess` from the OS fingerprint plugin.
  - **Protocol mix** — color by dominant protocol (node whose traffic is mostly DNS = DNS color, mostly HTTP = HTTP color). Could reuse `PROTOCOL_COLORS` from constants.
  - **Traffic volume** — gradient (cool→hot) based on total bytes or packet count. Highlights heavy talkers.
  - **Cluster membership** — color by cluster assignment (already done when clustering is active, but should be an explicit mode that persists cluster colors even when viewing individual nodes).
  - **Custom field** — color by any researcher metadata field (e.g. "role", "department", "risk_level" from the META JSON upload).
  Applies to node fill, NodeDetail badge, and legend. Each mode provides its own legend entries.
...
- [x] **Zero data loss alignment** (v0.10.5) — two architectural changes to align the codebase with the zero data loss principle (§1):
  1. **Caps → display layer.** All 21 `CAP_*` constants removed from `protocol_fields/*.py` and `sessions.py`. Data accumulates unbounded during session building. A shared `cap_list()` helper in `protocol_fields/__init__.py` applies a generous `SERIALIZE_CAP = 500` limit only at serialization time, adding a `_total` companion key when truncated (e.g. `dns_answers: [500 items], dns_answers_total: 873`). Dissector-level caps also removed (`dissect_dns.py` no longer truncates answer/authority/additional sections).
  2. **Lazy protocol init.** `all_init()` removed entirely. Protocol fields are lazy-initialized via try/except KeyError in `all_accumulate()` — when an accumulator first touches `s["dns_queries"]` and it doesn't exist, the init function runs and the accumulator retries. Sessions only contain fields for protocols actually seen. `_active_protocols` set tracks which protocols are live per session, cleaned up in `all_serialize()`.
- [x] **Session boundary detection** (v0.10.5) — `build_sessions()` now detects session boundaries within the same 5-tuple using three generic transport signals plus protocol-specific boundary checkers (OR logic): (1) TCP FIN/RST close followed by SYN reopen, (2) large timestamp gap (60s for UDP, 120s for TCP), (3) TCP sequence number jump >1M combined with >5s time gap (catches lost FIN/RST), (4) protocol-specific `check_boundary(flow_state, ex)` from protocol_fields modules — auto-discovered alongside init/accumulate/serialize. DHCP uses this to split on transaction ID (`dhcp_xid`) changes. Split sessions get suffixed IDs (`…#1`, `…#2`). Conservative thresholds — false non-splits preferred over false splits. Per-5-tuple flow state tracked alongside session accumulation with minimal overhead. First packet on each 5-tuple seeds flow state without triggering a split.
...
- [ ] **Full TCP stream reassembly** — current Follow TCP Stream shows first 128 bytes per packet. With a database backend, store full payloads and reassemble TCP streams properly (handle out-of-order segments, retransmissions, overlapping data). Enable full HTTP body viewing, file extraction preview, and protocol-aware stream parsing. Depends on database storage backend.
...
- [ ] **Smart capture selector (opening page)** — replace the current manual file-upload flow with an interactive opening page that queries a hypothetical backend for available data. User selects: time range (date picker), IP addresses/subnets (searchable multi-select), log sources (toggles per source type). Includes toggles like "include all IP connections" (pull in all sessions touching selected IPs, not just between them). Draws data from backend metadata index, displays summary stats before loading. Enables selective loading of large datasets instead of ingesting everything.
