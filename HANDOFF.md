...
- Session boundary detection: FIN/RST+SYN splits, timestamp gap splits (60s UDP, 120s TCP), seq jump heuristic
...
- [x] **Session boundary detection** (v0.10.5) — `build_sessions()` now detects session boundaries within the same 5-tuple using three heuristic signals: (1) TCP FIN/RST close followed by SYN reopen, (2) large timestamp gap (60s for UDP, 120s for TCP), (3) TCP sequence number jump >1M combined with >5s time gap (catches lost FIN/RST). Split sessions get suffixed IDs (`…#1`, `…#2`). Conservative thresholds — false non-splits preferred over false splits. Per-5-tuple flow state (`closed`, `last_ts`, `last_seq`) is tracked alongside session accumulation with minimal overhead.
