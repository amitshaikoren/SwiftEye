...
- [ ] **User-defined node color ranges** — currently node color is hardcoded to "private" (green) vs "external" (pink/red) based on RFC1918 ranges. Researchers may work with non-standard IP ranges (e.g. 100.70.x.x in a lab, CGNAT space, documentation ranges). Let the user define their own IP range → color mappings. Options: (1) a "Color Rules" panel where you add CIDR → color/label pairs (e.g. `100.70.0.0/16 → blue "Internal Lab"`), (2) import from researcher metadata JSON, (3) presets (RFC1918, cloud provider ranges, IANA special-purpose). Default stays RFC1918-based. Applies to both node fill color and the "External"/"Private" badge in NodeDetail.
