...
### v0.9.82 — March 2026
- **QUIC dissector (Phase 1)** — new protocol dissector for UDP port 443. Parses QUIC long headers to extract version, Destination/Source Connection IDs, and packet type (Initial/0-RTT/Handshake/Retry). Decrypts QUIC Initial packet header protection and payload using HKDF-derived keys from the DCID (RFC 9001 §5), then parses the contained CRYPTO frames to extract the TLS ClientHello SNI. Also extracts ALPN, supported TLS versions, and cipher suites from the ClientHello. Payload signature detection (`0x00000001` QUIC v1, `0x6b3343cf` QUIC v2 version bytes at offset 1) works on any UDP port. Session aggregation collects QUIC versions, connection IDs, SNIs, and ALPN protocols. Fields: `quic_version`, `quic_version_name`, `quic_dcid`, `quic_scid`, `quic_packet_type`, `quic_sni`, `quic_alpn`, `quic_tls_versions`, `quic_tls_ciphers`. Requires `cryptography` package (already optional dependency) for Initial packet decryption; falls back to header-only parsing without it.
